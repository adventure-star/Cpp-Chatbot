...
{
	bool bTransposed = 0;
	for(unsigned int i = 0; i < transposListSize; ++i)
	{
...
void CBot::find_match() 
{
	...
	// introduce thse new "string variable" to help support the implementation of keyword ranking during the matching process
	std::string bestKeyWord;
	std::vector<int> index_vector;

	for( unsigned int i = 0; i < nKnowledgeBaseSize; ++i) 
	{
		...
		// here introduce a second loop in order to support the 'keyword equivalence' feature
		for(unsigned int j = 0; j < keyWordList.size(); ++j)
		{
			...
			// inset a space character before and after the keyword to improve the matching process
			...
			if( keyPos != std::string::npos ) 
			{
				...
				// using the 'context feature' for selecting the best response possible
